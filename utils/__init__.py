...
from PersonBlogSystem.settings import MEDIA_URL
import random
import hashlib
import time
...
def get_constraint(view_func):  # 对视图函数做约束
    """
...
def login_required(views_func):
    ...
    def inner(request, *args, **kwargs):
        ...
        print(token)
        user = User.objects.filter(token=token)  # token设置的时间为3天
        print(user)
        if not user:  # 如果当期用户不存在或者token时间到期，重定向到登录页面，进行二次登录
